...
<template>
  <AppPageContainer>
    ...
    <div class="flex items-start justify-between mb-4 pb-3 border-b border-border">
      ...
      <button
        type="button"
        class="mt-1 flex items-center gap-1.5 h-9 px-3 rounded-md text-[13px] font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
        @click="openNew"
      >
        <Plus :size="16" />
        Nova meta
    ...
    <div v-if="store.loadingGoals" class="space-y-3">
      <div v-for="i in 3" :key="i" class="bg-card border border-border rounded-lg p-4">
        <div class="flex items-start gap-3">
          <div class="size-10 rounded-xl bg-muted/60 animate-pulse shrink-0" />
          <div class="flex-1 space-y-2">
            <div class="h-4 w-32 rounded bg-muted/60 animate-pulse" />
            <div class="h-3 w-48 rounded bg-muted/60 animate-pulse" />
    ...
    >
      <div class="size-14 rounded-2xl bg-muted/40 flex items-center justify-center">
        <Flag :size="24" class="text-muted-foreground/50" />
      </div>
      <div>
      ...
      <button
        type="button"
        class="mt-2 flex items-center gap-1.5 h-10 px-4 rounded-lg text-[13px] font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
        @click="openNew"
      >
        <Plus :size="16" />
        Criar meta
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <span
            ...
            :style="{ background: (goal.color ?? '#00C896') + '20' }"
          >
            <Flag :size="20" :style="{ color: goal.color ?? '#00C896' }" />
          </span>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between gap-2">
              ...
              <p class="text-[12px] text-muted-foreground/60 tabular-nums shrink-0">
            ...
            <div class="flex items-center gap-3 mt-1.5">
              ...
              <span v-if="goal.months_remaining !== null" class="text-[11px] text-muted-foreground/40">
                · {{ goal.months_remaining }} meses
              </span>
              <span
                v-if="goal.on_track === false"
                class="text-[10px] text-warning/70 font-medium"
    ...
    >
      ...
      <div class="space-y-2">
        <div
          ...
          :key="goal.id"
          class="bg-card border border-border rounded-lg px-4 py-3 flex items-center gap-3 opacity-60"
        >
          <span
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[11px] text-muted-foreground/50 capitalize">{{ goal.status === 'completed' ? 'Concluída' : 'Pausada' }}</p>
          </div>
          <p class="text-[12px] font-semibold tabular-nums text-success/70">
  ...
  <AlertDialog v-model:open="recurringDialogOpen">
    <AlertDialogContent class="max-w-sm">
      ...
      <AlertDialogFooter class="gap-2">
        <AlertDialogCancel
          class="flex-1 h-[52px] rounded-xl text-[15px] bg-muted/60 border border-border/50 text-muted-foreground"
