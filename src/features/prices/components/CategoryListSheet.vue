  ...
  <Sheet :open="open" @update:open="emit('update:open', $event)">
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto pb-8">
        ...
        <p v-else-if="store.categories.length === 0" class="text-[13px] text-muted-foreground/60 px-5 py-8 text-center">
          ...
          >
            ...
            <template v-if="editingId === category.id">
              <input
                v-model="editName"
                type="text"
                class="flex-1 h-9 rounded-lg bg-card border border-border/60 px-3 text-[13px] text-foreground outline-none transition-colors focus:border-primary/60"
